...
const AdminDashboardPage: React.FC = () => {
  ...
  const handleSaveApp = async (
    ...
    try {
      ...
      if (files.iconFile) {
        ...
        submissionData.icon_url = await uploadAppFile(files.iconFile, appData.name);
      }
      ...
      if (files.apkFile) {
        ...
        submissionData.apk_url = await uploadAppFile(files.apkFile, appData.name);
      }

      if (editingApp) {
        const urlsToDelete = (editingApp.screenshots || []).filter(url => !(appData.screenshots || []).includes(url));
        if (urlsToDelete.length > 0) {
          await Promise.all(urlsToDelete.map(url => deleteAppFileByUrl(url)));
        }
      }

      const newScreenshotUrls = files.screenshotFiles?.length ? await Promise.all(
  ...
  const handleCreateProfile = async () => {
    ...
    try {
        ...
        setError("Imeshindwa kutengeneza wasifu wako. Tafadhali jaribu tena.");
    } finally {
  ...
  const getRoleDisplayName = (role: Profile['role']) => ({'admin': 'Msimamizi', 'developer': 'Msanidi', 'user': 'Mtumiaji'})[role] || role;
  ...
  return (
    <div className="min-h-screen bg-slate-100">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
            ...
            <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-r-lg shadow-md mb-8">
                <div className="flex">
                    ...
                    <div>
                        ...
                        <button onClick={handleCreateProfile} disabled={isCreatingProfile} className="mt-3 bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-4 rounded-lg transition-colors text-sm disabled:bg-yellow-300">
                            {isCreatingProfile ? "Inatengeneza..." : "Tengeneza Wasifu (Admin)"}
                        </button>
                    </div>
                </div>
            </div>
        ...
        <div className="bg-surface p-6 rounded-lg shadow-lg">
            <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => handleOpenAppModal()} className="bg-primary hover:bg-blue-700 text-white font-bold py-2 px-5 rounded-lg transition-colors">
                    Ongeza Programu
                </button>
            </div>
            {loading ? <p>Inapakia...</p> : error ? <p>{error}</p> : (
                <AppTable apps={apps} profile={profile} onEdit={handleOpenAppModal} onDelete={handleDeleteApp} onStatusChange={handleStatusChange} onAnalyze={handleAnalyzeApp} />
            )}
        </div>
        
        {profile?.role === 'admin' && (
          <div className="bg-surface p-6 rounded-lg shadow-lg">
              <h2 className="text-xl font-semibold text-gray-700 mb-6">Usimamizi wa Watumiaji</h2>
              {loadingUsers ? <p>Inapakia...</p> : userError ? <p>{userError}</p> : (
                      ...
                      <table className="min-w-full bg-white">
                          <thead className="bg-slate-50">
                              <tr>
                                  ...
                                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Cheo</th>
                                  <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">Vitendo</th>
                              </tr>
                          </thead>
                          ...
                          {users.map(u => (
                              <tr key={u.id}>
                                  ...
                                  <td className="px-6 py-4 text-sm">{getRoleDisplayName(u.role)}</td>
                                  <td className="px-6 py-4 text-right text-sm font-medium">
                                      <button onClick={() => handleOpenUserModal(u)} className="text-indigo-600 hover:text-indigo-900 mr-4">Hariri</button>
                                      <button onClick={() => handleDeleteUser(u.id)} className="text-red-600 hover:text-red-900 disabled:text-gray-400" disabled={u.id === profile?.id}>Ondoa</button>
                                  </td>
                              </tr>
                          ))}
                          </tbody>
      ...
      {isAppModalOpen && <Modal title={editingApp ? 'Hariri Programu' : 'Ongeza Programu'} onClose={handleCloseAppModal}><AppForm onSubmit={handleSaveApp} initialData={editingApp} /></Modal>}
      {isUserModalOpen && editingUser && <UserEditModal user={editingUser} onClose={handleCloseUserModal} onSave={handleSaveUserRole} />}
      {isAnalysisModalOpen && analyzingApp && (
          <Modal title={`Uchambuzi wa AI kwa "${analyzingApp.name}"`} onClose={() => setIsAnalysisModalOpen(false)}>
              {isAnalyzing ? (
                  <div className="flex justify-center items-center h-40"><div className="animate-spin rounded-full h-16 w-16 border-t-2 border-b-2 border-primary"></div><p className="ml-4">Inachambua...</p></div>
              ) : (
                  <div className="space-y-4"><h3 className="font-semibold">Matokeo:</h3><div className="bg-slate-50 p-4 rounded-lg border whitespace-pre-wrap"><p>{analysisResult}</p></div></div>
              )}
          </Modal>
      )}
    </div>
